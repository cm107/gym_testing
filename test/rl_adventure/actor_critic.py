...
from gym_testing.rl_adventure.util import get_device, make_dir_if_not_exists
from gym_testing.rl_adventure.env_util import make_env, make_env_list
from gym_testing.rl_adventure.network.actor_critic import ActorCritic
...
class ActorCriticWorker:
    ...
    def infer(self, num_frames: int=100, delay: float=1/20):
...
for num_steps in [5, 20]:
    for use_gae in [True, False]:
        for env_name in [
            ...
            if use_gae:
                run_id = f"{run_id}-gae"
            worker = ActorCriticWorker(env_name=env_name, run_id=run_id)
            worker.train(num_steps=num_steps, use_gae=use_gae)
            # worker.infer(num_frames=500)