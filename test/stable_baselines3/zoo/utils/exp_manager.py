...
class ExperimentManager(object):
    ...
    def create_envs(self, n_envs: int, eval_env: bool = False, no_log: bool = False) -> VecEnv:
        ...
        print(f'self.env_id: {self.env_id}') # SpaceInvaders-v0
        print(f'n_envs: {n_envs}') # 1
        print(f'self.seed: {self.seed}') # 4208325217
        print(f'self.env_kwargs: {self.env_kwargs}') # {}
        print(f'log_dir: {log_dir}') # None
        print(f'self.env_wrapper: {self.env_wrapper}') # AtariWrapper
        print(f'self.vec_env_class: {self.vec_env_class}') # DummyVecEnv
        print(f'self.vec_env_kwargs: {self.vec_env_kwargs}') # {}
        print(f'monitor_kwargs: {monitor_kwargs}') # {}
        import sys
        sys.exit()
